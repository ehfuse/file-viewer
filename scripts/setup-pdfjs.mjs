#!/usr/bin/env node
/**
 * PDF.js 뷰어 자산 세팅 스크립트 (소비 프로젝트의 postinstall 등에서 실행, 크로스플랫폼 node).
 *
 * 목적: FileViewer 의 PDF 미리보기가 쓰는 worker/cmaps/표준폰트/wasm 을
 *       설치된 pdfjs-dist 버전 그대로 정적 폴더에 복사한다.
 *       CDN 고정 버전을 쓰면 pdfjs-dist 업그레이드 시 API/Worker 버전이 어긋나
 *       "PDF 파일을 불러올 수 없습니다." 로 조용히 깨진다(CDN 차단 환경도 대비).
 *
 * 사용법: file-viewer-setup-pdfjs [대상폴더]   (기본: public/pdfjs)
 *   예) "postinstall": "file-viewer-setup-pdfjs public/pdfjs"
 * 복사 후 FileViewer 의 pdfAssetBase 를 대상 폴더의 런타임 경로(기본 "/pdfjs")로 맞춘다.
 */
import { existsSync, mkdirSync, copyFileSync, readdirSync, rmSync } from "node:fs";
import { createRequire } from "node:module";
import { dirname, join, resolve } from "node:path";

// 대상 폴더: 첫 번째 인자, 없으면 실행 위치 기준 public/pdfjs
const DST = resolve(process.cwd(), process.argv[2] || join("public", "pdfjs"));

/**
 * FileViewer 는 `import { pdfjs } from "react-pdf"` 로 pdfjs API 를 쓰므로,
 * worker 는 **react-pdf 가 실제로 해석하는** pdfjs-dist 여야 한다.
 * react-pdf 는 pdfjs-dist 를 정확한 버전으로 고정(핀)하기 때문에, 프로젝트가 pdfjs-dist 를
...
// pdfjs-dist 자산을 대상 폴더로 복사한다(항상 — 설치 버전과 자동 일치)
function copyAssets() {
    if (!SRC || !existsSync(SRC)) {
        console.warn("[file-viewer] pdfjs-dist 미설치 — 자산 복사 건너뜀");
        return;
    }
    ...
    for (const dir of DIRS) {
        ...
        if (!existsSync(from)) {
            console.warn(`[file-viewer] ${dir} 없음 — 건너뜀`);
            continue;
        }
    ...
    for (const parts of FILES) {
        ...
        if (!existsSync(from)) {
            console.warn(`[file-viewer] ${parts.join("/")} 없음 — 건너뜀`);
            continue;
        }
        copyFileSync(from, join(DST, parts[parts.length - 1]));
    }

    console.log(`[file-viewer] pdfjs-dist v${version} 자산 복사 완료 → ${DST}`);
}
